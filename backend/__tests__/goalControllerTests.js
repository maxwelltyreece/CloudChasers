...
// import { expect, it, describe, beforeEach, beforeAll, afterAll, afterEach } from 'jest';
...
describe('Award Controller Tests', () => {
    ...
    describe('Create goal', () => {
		...
		it('should fail to create a goal whilst missing ', async () => {
			const res = await request(app)
				.post('/goals/createGoal')
				.set('Authorization', `Bearer ${token}`)
				.send({
			...
			expect(res.statusCode).toEqual(400);
			expect(res.body.message).toEqual('Missing required fields: minTargetMass');
		});
	});
});